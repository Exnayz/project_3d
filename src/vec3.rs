...
impl Vec3 {
    ...
    pub fn mul_n(&self, num: f32) -> Vec3 { Vec3 { x: self.x * num, y: self.y * num, z: self.z * num }}
    ...
    pub fn norm(&self) -> Vec3 { self.mul_n(1.0 / self.lenght()) }
    pub fn lenght(&self) -> f32 { f32::sqrt(self.x*self.x + self.y*self.y + self.z*self.z) }
    pub fn cos_angle(&self, v: Vec3) -> f32 { self.dot(v) / (self.lenght() * v.lenght()) }
